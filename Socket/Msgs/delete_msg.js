...
const deleteMsg = async (MsgId, socket, io, onlineUser) => {
    try {
        ...
        if (!FindMsg) return socket.emit('delete-message-error', {
        ...
        if (FindMsg.senderId.toString() !== requestingUserId) {
            return socket.emit('delete-message-error', {
        ...
        if (FindMsg.receiverId) {
            ...
            if (!group) return socket.emit('delete-message-error', {
                message: 'group dont found!'
            });
            ...
            group.members.forEach(m => {
                ...
                if (onlineUser.has(userid)) {
                    for (let socketId of onlineUser.get(userid)) {
                        socket.to(socketId).emit('message-deleted', {
                            messageId: MsgId,
                            message: 'Message deleted!'
                        })
                    }
        ...
        const softDelete = async (retry = 3, delay = 100) => {
            try {
                ...
                if (retry > 0) {
                    setTimeout(() => {
                        softDelete(retry - 1, delay)
                    }, delay);
                } else {
                    return socket.emit('delete-message-error', {
                        message: 'Somthing went wrong while delete file!'
                    })
                }
            }
        }
        ...
        softDelete()
        socket.emit('delete-sucessfully', {
            message: 'Message delete succesfully!'
        })

    } catch (error) {
        return socket.emit('delete-message-error', {
            message: 'Somthing went wrong, while deleting message!'
        })
    }
}
